...
public class BotState {
	...
	private ArrayList<Region> pickableStartingRegions; // list of regions the
														// player can choose the
														// start from
	private ArrayList<Region> wastelands; // wastelands, i.e. neutral regions
											// with a larger amount of armies on
											// them. Given before the picking of
											// starting regions
	...
	// the number of armies in the SuperRegion
	public int getArmies(SuperRegion superRegion) {
		int count = 0;
		for (Region region: superRegion.getSubRegions()) {
			count += (wastelands.contains(region)) ? 6 : 2;
		}
		// /
		System.err.println("Super Region: " + superRegion.getId() + " has " + count + " armies");
		// /
		return count;
	}
